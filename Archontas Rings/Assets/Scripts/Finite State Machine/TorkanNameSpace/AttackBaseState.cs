    ...
    namespace Torkan
    {
        public class AttackBaseState : State
        {
            ...
            private void DoAttack(string attack_name)
            {
                ...
                stateMachine.enemyAnimationManager.PlayTargetAnimation(attack_name, true, attack_name.Contains("RM"));
            }
            ...
            private bool AttackConditions(int min, int max)
            {
                if (stateMachine.canAttack)
                {
            ...
            public void ChanceAttack(int min, int max, string[] listOfAttacks)
            {
                if (AttackConditions(min, max))
                {
                    DoAttack(listOfAttacks[Random.Range(0, listOfAttacks.Length - 1)]);
                }
            }
